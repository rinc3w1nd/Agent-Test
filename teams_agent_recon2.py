...
async def install_control_overlay(page: Page, cfg: dict, bot_name: str):
    ...
    async def _py_record_status():
        ...
        try:
            ...
            text = ""
        if not text:
            note = await page.evaluate("window.prompt('No bot reply detected. Enter a short note for the log (optional):')")
        else:
            note = ""
...
(() => {
  ...
  stat.onclick = async () => {
    ...
    setMsg('Status [' + (r && r.ts || '-') + ']: shot=' + (r && r.screenshot || '-') + (r && r.note ? ('; note: '+r.note) : ''));
  };
})();
"""
